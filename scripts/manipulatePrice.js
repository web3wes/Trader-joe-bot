...
const main = async () => {
    ...
    web3.eth.getBalance(account, function(err, result) {
        if (err) {
          ...
          console.log(web3.utils.fromWei(result, "ether") + " ETH in account")
        }
      })
      ...
      async function tx() {
        ...
        web3.eth.getBalance(UNLOCKED_ACCOUNT, function(err, result) {
            if (err) {
              console.log(err)
            } else {
              console.log(web3.utils.fromWei(result, "ether") + " ETH in unlock before")
            }
          })

        web3.eth.sendTransaction({to: UNLOCKED_ACCOUNT, from: account, value: web3.utils.toWei('10', 'ether')});
    ...
    let shib_balance = await ERC20_CONTRACT.methods.balanceOf(UNLOCKED_ACCOUNT).call()
    shib_balance = web3.utils.fromWei(shib_balance.toString(), 'ether')


    console.log(`\nShibBalance in reciever account: ${shib_balance} SHIB\n`)
}
